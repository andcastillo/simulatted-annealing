...
const LM = require('ml').levenbergMarquardt;
...
async function run(mol, jdx) {
    ...
    let minValues = initialValues.map(value => value - 0.1);
    let maxValues = initialValues.map(value => value + 0.1);
    ...
    let fittedParams = LM(data, simulatedSpectrum, options);
