...
async function run(mol, jdx) {
    ...
    let fittedParams = runFitting(data, spinSystem, smoothing, options1h, 0, 2000);
...
function predictParameters(predictor, sample) {
    ...
    let h1 = predictor.proton(molecule, { OCLE, group: true, ignoreLabile: false, levels: [5, 4, 3] });
 ...
 */
function runFitting(data, spinSystem2, smoothing, options1h, quenchingFactor, maxIterations) {
    ...
    const options = {
        ...
        minValues,
        maxValues,
        quenchingFactor: quenchingFactor,
    ...
    let fittedParams = SA(data, fx, options);
