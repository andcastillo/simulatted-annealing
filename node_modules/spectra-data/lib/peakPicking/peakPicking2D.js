...
function getZones(spectraData, thresholdFactor) {
  ...
  if (isHomonuclear) {
    let convolutedSpectrum = _mlFft.FFTUtils.convolute(data, smallFilter, nbSubSpectra, nbPoints);
    let peaksMC1 = _mlMatrixPeaksFinder2.default.findPeaks2DRegion(data, { filteredData: convolutedSpectrum, rows: nbSubSpectra, cols: nbPoints, nStdDev: nStdDev * thresholdFactor }); // )1.5);
    ...
    let convolutedSpectrum = _mlFft.FFTUtils.convolute(data, smallFilter, nbSubSpectra, nbPoints);
    let peaksMC1 = _mlMatrixPeaksFinder2.default.findPeaks2DRegion(data, { filteredData: convolutedSpectrum, rows: nbSubSpectra, cols: nbPoints, nStdDev: nStdDev * thresholdFactor });
    ...
    return createSignals2D(_peakOptimizer2.default.clean(peaksMC1, 0.05), spectraData, 24);
  }
}
